...
对于 total language $$ L $$, 有图灵机 $$ i $$, $$ [\![p]\!]_L(d) := i(p . d) $$. 假设存在解释器 $$ p $$, $$ i(p . (q . d)) = [\![p]\!]_L(q . d) = i(q . d) $$, 那么考虑程序 $$ [\![s]\!]_L(r) := [\![p]\!]_L(r . r) + 1 $$, 有 $$ [\![p]\!]_L(s . s) = [\![s]\!]_L(s) = [\![p]\!]_L(s . s) + 1 $$, 矛盾. 这里假设 $$ L $$ 可以表达 pair 和 +1 (或任意类似函数).
